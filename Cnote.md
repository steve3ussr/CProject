...
1. 在函数或块内部的**局部**变量(Parameter)
2. 在所有函数外部的**全局**变量(parameter)
3. 在**形式**参数(Argument)的函数参数定义中
...
> [一些工程中的例子](https://zhuanlan.zhihu.com/p/389374215)

我觉得枚举的作用就是，免除了一些宏定义。

``` C++
enum EnumName[OPTIONAL] {
    frewe, 
    gtrge, 
    gtjy = 6, 
    yhe
...
或者`enum EnumName {}varName`;

这种情况下，定义一个雷剧类型的变量需要写成`enum EnumName var;`

如果在定义枚举的时候写成`typedef enum ... {};`的话，以后声明枚举类变量的时候就可以不写enum了。

- C里默认为`uint`类型；
- 其中枚举类名不是必须的；
- 不指定的话，默认第一个对应0；
- 不指定的话，每个对应的值在前一个的基础上+1；
- 上面例子中的分别为0，1，2，6，7
- 以后在数组引用，或者其他什么的时候可以直接用枚举的名字代替u8.
- 如果在枚举类的后面加一个额外的量，可以用作枚举类的数量（不更改默认值的情况下）
- *在枚举连续的情况下，可以遍历，但是好像没啥用*
...
`struct StructName{} inst; inst.a; &inst->a;`
